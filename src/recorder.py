...
def recordData(proj_list):
    ...
    for fn in proj_list:
        ...
        for line in lines:
            ...
            if str(json_data['label-type'][0]) not in token_choices:
                print('added choice \"' + str(json_data['label-type'][0]) + '\"')
                token_choices.append(str(json_data['label-type'][0]))

            for tok in json_data['prefix']:
                if str(tok) not in token_choices:
                    print('added choice \"' + str(tok) + '\"')
                    token_choices.append(str(tok))
